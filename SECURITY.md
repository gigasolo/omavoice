...
- Meeting echo cancel uses `monitor.mode`, so it can read the session default sink (speakers or headphones) while that graph is running.
...
- Calls `omarchy-audio-input-set-default` to make `omavoice` the default mic, and restores the last real capture (`previousAudioSource`) when disabled. It never stores `omavoice` as that previous source.
