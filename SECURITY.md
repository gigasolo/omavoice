...
- Meeting echo cancel uses `monitor.mode`, so it can read the session **default sink** — speakers or Bluetooth headphones — while a real client is capturing Omavoice. The panel hold and Quickshell peak meters do not count. With no listener, those monitor links are dropped.
