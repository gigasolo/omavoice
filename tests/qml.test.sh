...
grep -q '"version": "0.1.15"' "$root/manifest.json" || fail "manifest must be 0.1.15"
grep -q 'omavoice.png' "$root/OmavoiceIcon.qml" || fail "icon must use the rendered orb"
grep -q 'docs/omavoice.png' "$root/README.md" || fail "README must use the rendered banner"
...
grep -q 'function showTune' "$root/Panel.qml" || fail "preset wrench must flip to a tune page"
grep -q 'id: presetWrench' "$root/Panel.qml" || fail "PRESET needs a wrench"
grep -q 'id: defaultMicSwitch' "$root/Panel.qml" || fail "MICROPHONE needs an Auto toggle"
...
test -f "$root/omavoice.png" || fail "omavoice.png mark is missing"
test -f "$root/docs/omavoice.png" || fail "README banner PNG is missing"
