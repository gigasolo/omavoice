...
grep -q '"version": "0.1.9"' "$root/manifest.json" || fail "manifest must be 0.1.9"
grep -q 'id: heroIcon' "$root/Panel.qml" || fail "header must use a HEY-sized microphone"
grep -q 'id: powerButton' "$root/Panel.qml" || fail "power must be an icon button like settings"
grep -q 'ToggleSwitch' "$root/Panel.qml" && fail "header must not mix a switch with icon buttons"
