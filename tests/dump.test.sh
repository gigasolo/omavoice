...
echo "$podcast" | grep -A5 'name = eq_body' | grep -q '"Gain" = -2.5' || fail "podcast Presence body must be -2.5"
echo "$podcast" | grep -A5 'name = eq_pres' | grep -q '"Gain" = 2.0' || fail "podcast Presence mid must be +2.0"
echo "$clean" | grep -q 'name = eq_body' && fail "clean must not emit voice EQ"
meeting_air="$(dump meeting --eq air)"
echo "$meeting_air" | grep -A4 'name = hp' | grep -q '"Freq" = 100.0' || fail "Air curve must raise HPF to 100 Hz"
echo "$meeting_air" | grep -A5 'name = eq_air' | grep -q '"Gain" = 2.5' || fail "Air curve air gain must be +2.5"
