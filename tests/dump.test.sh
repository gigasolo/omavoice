...
echo "$meeting" | grep -q 'name = preamp' || fail "meeting must name preamp"
echo "$meeting" | grep -q 'name = outgain' || fail "meeting must name outgain"
echo "$meeting" | grep -q 'name = eq.body' || fail "meeting must stub eq.body"
echo "$meeting" | grep -q 'name = eq.presence' || fail "meeting must stub eq.presence"
echo "$meeting" | grep -q 'name = eq.air' || fail "meeting must stub eq.air"
echo "$meeting" | grep -q 'label = copy' || fail "gain nodes must be builtin copy"
echo "$meeting" | grep -q '"Gain" = 1.0' || fail "copy Gain default 1.0"
echo "$meeting" | grep -q 'output = "lim:out" input = "outgain:In"' || fail "outgain after limiter"
echo "$meeting" | grep -q 'inputs = \[ "preamp:In" \]' || fail "graph input is preamp"
echo "$meeting" | grep -q 'outputs = \[ "outgain:Out" \]' || fail "graph output is outgain"
echo "$meeting" | grep -q '"Freq" = 180' || fail "eq.body default Freq 180"
echo "$meeting" | grep -q '"Q" = 0.7' || fail "eq.body default Q 0.7"
echo "$meeting" | grep -q '"Freq" = 2500' || fail "eq.presence default Freq 2500"
echo "$meeting" | grep -q '"Q" = 0.9' || fail "eq.presence default Q 0.9"
echo "$meeting" | grep -q '"Freq" = 7000' || fail "eq.air default Freq 7000"

echo "$podcast" | grep -q 'name = preamp' || fail "podcast must name preamp"
echo "$podcast" | grep -q 'name = outgain' || fail "podcast must name outgain"
echo "$podcast" | grep -q 'name = eq.body' || fail "podcast must stub eq.body"
echo "$podcast" | grep -q 'inputs = \[ "preamp:In" \]' || fail "podcast graph input is preamp"
echo "$podcast" | grep -q 'outputs = \[ "outgain:Out" \]' || fail "podcast graph output is outgain"

echo "$clean" | grep -q 'name = preamp' || fail "clean must still have preamp"
echo "$clean" | grep -q 'name = outgain' || fail "clean must still have outgain"
echo "$clean" | grep -q 'name = eq.body' && fail "clean must not grow EQ stubs"
echo "$clean" | grep -q 'noise_suppressor' && fail "clean must not denoise"
echo "$clean" | grep -q 'inputs = \[ "preamp:In" \]' || fail "clean graph input is preamp"
echo "$clean" | grep -q 'outputs = \[ "outgain:Out" \]' || fail "clean graph output is outgain"
echo "$clean" | grep -q 'output = "hp:Out" input = "outgain:In"' || fail "clean hp feeds outgain"
